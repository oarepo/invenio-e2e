 ...
 */
export type UseFunction<L extends Locators, T extends typeof BasePage<L>> = (
  r: InstanceType<T>
) => Promise<void>;
 ...
 * a page fixture.
 */
export type FixtureRegistrationFunction<
  L extends Locators,
  T extends typeof BasePage<L>,
> = (params: PageFixtureParams<L>, use: UseFunction<L, T>) => Promise<void>;
...
export function registerPage<L extends Locators, T extends typeof BasePage<L>>(
  ...
  if (options.extraFixtures?.length > 0) {
    ...
    const func = eval(
      `async ({ ${parameters} } , use) => { return await pageCreator({ ${parameters} }, use); }`
    );
    return {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      [name]: func,
  ...
  return {
    [name]: async (
      { page, locators, availablePages, services, expect }: PageFixtureParams<L>,
      use: UseFunction<L, T>
    ) => {
