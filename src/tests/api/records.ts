...
export function recordsApiTests(
  test: InvenioTest,
  authUserFilePath: string = path.resolve(
    appConfig.e2eRootPath,
    appConfig.authUserFilePath
  ),
  authAdminFilePath: string = path.resolve(
    appConfig.e2eRootPath,
    appConfig.authAdminFilePath
  ),
  ...
  test.afterAll(async () => {
    await Promise.allSettled([userApiContext.dispose(), adminApiContext.dispose()]);
  });
  ...
  const testFiles: FileUploadDescriptor[] = [
    ...
    {
      key: "logo-invenio-rdm.svg",
      path: "https://inveniordm.docs.cern.ch/images/logo-invenio-rdm.svg",
  ...
  const buildRecordObjectMatchers = (
  ...
  const createDraftRecord = async (
    ...
    recordObjectMatchers: Record<string, unknown>;
  }> => {
    ...
    expect(createdRecord, "should be a draft and match the expected structure").toEqual(
      expect.objectContaining(draftRecordMatchers)
    );
  ...
  const publishAndVerifyRecord = async (
    createdRecord: ApiRecordResponse,
    recordObjectMatchers: Record<string, unknown>
  ) => {
    // Publish the record
    ...
    expect(
      publishedRecord,
      "should be published and match the expected structure"
    ).toEqual(
    ...
    expect(
      recordData,
      "should return again the published record with correct structure"
    ).toEqual(
      expect.objectContaining({
        id: createdRecord.id,
        status: "published",
  ...
  const uploadFileWithLocalTransfer = async (
    createdRecord: ApiRecordResponse,
    ...
    const startFilesUploadResponse = await userApiContext.post(
      createdRecord.links.files,
      {
        data: [{ key: file.key }],
      }
    );
    ...
    const startFilesUploadData =
      (await startFilesUploadResponse.json()) as StartUploadResponse;

    expect(startFilesUploadData, "should be pending and have links section").toEqual(
      expect.objectContaining({
    ...
    expect(
      contentUrl,
      `content upload link should exist for ${file.key}`
    ).toBeDefined();
    ...
    // Commit file upload
    const commitFileResponse = await userApiContext.post(
      startFilesUploadData.entries[0].links.commit
    );

    expect(commitFileResponse.status()).toBe(200);

    const committedFileData = (await commitFileResponse.json()) as Record<
      string,
      unknown
    >;
    ...
    expect(
      committedFileData,
      "should be completed and have valid checksum after commit"
    ).toEqual(
      expect.objectContaining({
        key: file.key,
        status: "completed",
  ...
  const uploadFileWithFetchTransfer = async (
    createdRecord: ApiRecordResponse,
    file: FileUploadDescriptor
  ) => {
    ...
    const startFetchUploadResponse = await adminApiContext.post(
      createdRecord.links.files,
      {
        data: [
          {
    ...
    const startFetchUploadData =
      (await startFetchUploadResponse.json()) as StartUploadResponse;

    expect(
      startFetchUploadData,
      `file ${file.key} should be pending with fetch method`
    ).toEqual(
      expect.objectContaining({
        entries: expect.arrayContaining([
          expect.objectContaining({
    ...
    const fetchedFileEntry = startFetchUploadData.entries.find(
    ...
    const commitFetchResponse = await adminApiContext.post(
      fetchedFileEntry!.links.commit
    );

    expect(commitFetchResponse.status()).toBe(200);

    const committedFetchData = (await commitFetchResponse.json()) as Record<
      string,
      unknown
    >;
    ...
    const remoteFileStream = Readable.fromWeb(
      remoteFileResponse.body as ReadableStream
    );
    ...
    expect(
      committedFetchData,
      `commit of ${file.key} should be completed with valid checksum`
    ).toEqual(
      expect.objectContaining({
        key: file.key,
        status: "completed",
  ...
  const uploadFileWithMultipartTransfer = async (
    ...
    const startMultipartTransferResponse = await userApiContext.post(
      createdRecord.links.files,
      {
        data: [
          {
    ...
    const startMultipartTransferData =
      (await startMultipartTransferResponse.json()) as StartUploadResponse;

    expect(
      startMultipartTransferData,
      `file ${file.key} should be pending with multipart method`
    ).toEqual(
      expect.objectContaining({
        entries: expect.arrayContaining([
          expect.objectContaining({
    ...
    const multipartEntry = startMultipartTransferData.entries.find(
      (entry) => entry.key === file.key
    );
    expect(multipartEntry, `expected upload entry for ${file.key}`).toBeDefined();
    const partLinks = multipartEntry!.links.parts ?? [];

    expect(partLinks.length, `should have ${partsCount} parts as specified`).toBe(
      partsCount
    );
    ...
    const partsUploadResponses = await Promise.all(
      partLinks.map((part, index) => {
        ...
        const partEnd =
          index === partLinks.length - 1
            ? multipartFileContent.length
            : Math.min(
                (index + 1) * appConfig.s3DefaultBlockSize,
                multipartFileContent.length
              );
    ...
    for (let i = 0; i < partsUploadResponses.length; i++) {
      expect(
        partsUploadResponses[i].status(),
        `upload of part ${i + 1} should be successful`
      ).toBe(200);
    }

    const commitMultipartResponse = await userApiContext.post(
      multipartEntry!.links.commit
    );
