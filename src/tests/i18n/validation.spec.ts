...
function readValidationReport(): ValidationSummary | null {
  const reportPath = path.join(__dirname, '../../translations/validation-report.json');
...
test.describe('Translation validation (fast, no browser)', () => {
  test('Completeness and quality thresholds', async () => {
    ...
    if (!summary) test.skip(true, 'Validation report missing. Run `npm run collect-translations:validate`.');
    ...
    for (const r of reports) {
      if (!perLocaleTotals.has(r.locale)) perLocaleTotals.set(r.locale, { total: 0, untranslated: 0 });
      const a = perLocaleTotals.get(r.locale)!;
      a.total += r.counts.untranslated + r.counts.fuzzyTranslations + r.counts.obsoleteTranslations;
    ...
    for (const [locale, stats] of perLocaleTotals.entries()) {
      if (stats.total > 0) {
        ...
        expect(coverage, `${locale} coverage too low: ${coverage.toFixed(1)}%`).toBeGreaterThanOrEqual(20);
      }
    }

    // Fuzzy translations overall threshold
    expect(s.fuzzyTranslations, `Too many fuzzy translations (${s.fuzzyTranslations}).`).toBeLessThanOrEqual(5000);
  });
});
